...
def gradfx(X, A, b):
    ...
    for i in np.arange(n):
        Z[i, i] = 1 / (sum(exp(dot(A[i, :], X))))
    return dot(A.T, (dot(Z, exp(dot(A, X))) - b))
...
def stocgradfx(X, minibatch_size, A, b):
    ############## YOUR CODES HERE ##############
    n=A.shape[0]
    samples=np.random.randint(0,n,minibatch_size)
    grad=gradfx(X,A[samples,:],b[samples,:])
    return (grad*n)/minibatch_size
