...
def write_train_test_mnist(A_train, b_train, b_train_binarized, A_test, b_test):
    with open(MNIST_TRAIN_TEST_SPLIT, 'wb') as f:
        np.savez_compressed(f, A_train=A_train, b_train=b_train, b_train_binarized=b_train_binarized,
                            A_test=A_test, b_test=b_test)
    print('----- Successfully compressed and wrote {:s}.'.format(MNIST_TRAIN_TEST_SPLIT))


def write_results(results_dictionary):
    with open(RESULTS_FILE, 'wb') as f:
        pickle.dump(results_dictionary, f)
    print('----- Successfully stored results.')
...
def preprocess_data(mnist_bunch, rand_seed, train_size, test_size):
    A_train, A_test, b_train, b_test = train_test_split(mnist_bunch['data'], mnist_bunch['target'],
                                                        train_size=train_size,
                                                        test_size=test_size, random_state=rand_seed)

    A_train = A_train / 255.0
    A_test = A_test / 255.0
    b_train = np.array([int(val) for val in b_train])
    b_test = np.array([int(val) for val in b_test])

    label_binarizer = preprocessing.LabelBinarizer()
    label_binarizer.fit(b_train)
    b_train_binarized = label_binarizer.transform(b_train)
    num_classes = len(label_binarizer.classes_)
    num_features = A_train.shape[1]

    print('----- Preprocessing done.\n')

    return A_train, b_train, b_train_binarized, A_test, b_test, num_classes, num_features


def prepare_data(rand_seed, train_size, test_size):
    mnist = fetch_mnist()
    A_train, b_train, b_train_binarized, A_test, b_test, _, _ = preprocess_data(mnist, rand_seed, train_size, test_size)
    write_train_test_mnist(A_train, b_train, b_train_binarized, A_test, b_test)
