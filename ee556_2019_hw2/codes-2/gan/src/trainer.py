...
class GanTrainer():
    ...
    def objective(f, g, data_sample, noise_sample):
        ...
        # TODO
        dis1=f(data_sample)
        dis2=f(g(noise_sample))
        dist=dis1-dis2
        return dist.mean()#difine in the equation2, X means the data sample and Z means the noise sample
    ...
    def simultaneous_update(self, f, g, f_optim, g_optim):
        ...
        """
        # TODO
        data_sample=self.data.sample((self.batch_size,))#sample
        noise_sample=self.noise.sample((self.batch_size,))
        f_optim.zero_grad()#set the grad as 0 to avoid the automatic tracking
        g_optim.zero_grad()
        self.objective(f, g, data_sample, noise_sample).backward()# backward algorithm
        f_optim.step()#update optimizer
        g_optim.step()
    ...
    def alternating_update(self, f, g, f_optim, g_optim):
        ...
        noise_sample = self.noise.sample((self.batch_size,))
        f_optim.zero_grad()# updata optimizer alternately
        self.objective(f,g,data_sample,noise_sample).backward()
        f_optim.step()
        g_optim.zero_grad()
        self.objective(f, g, data_sample, noise_sample).backward()
        g_optim.step()
